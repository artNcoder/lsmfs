...
#SEARCH_DIR="/data/HDD_Sdd_data/lsmfs_test/linux"    #LSMFS
SEARCH_DIR="/data/HDD_Sdc_XFS_data/linux"       #XFS_HDD

TARGET_FILE="dio-sysfs.c" # 目标文件名
...
time_file_search() {
    ...
    start_time=$(date +%s.%N)
    find "$SEARCH_DIR" -type f -name "$TARGET_FILE" >/dev/null
    end_time=$(date +%s.%N)
